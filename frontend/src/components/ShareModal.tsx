      ...
      >
        ...
        <div className="mb-4 flex items-start justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-lg font-semibold">
              <Share2 className="h-5 w-5 text-primary-400" /> Share Monitor
            </h2>
            <p className="text-sm text-slate-400">Control who can access this monitor</p>
          </div>
        ...
        <div className="mb-4 space-y-3 border-b border-white/10 pb-4">
          ...
          {availableUsers.length === 0 && (
            <p className="text-xs text-slate-400">No other users available to share with.</p>
          )}
          ...
          <div className="space-y-1.5">
            <label className="flex items-center gap-2 text-sm">
              ...
              />
              Read-only <span className="text-xs text-slate-400">(view &amp; alerts)</span>
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="radio"
              ...
              />
              Can edit <span className="text-xs text-slate-400">(modify, pause, maintenance)</span>
            </label>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {sharesLoading ? (
            ...
            <div className="space-y-2">
              {shares.map((s) => (
                <div key={s.shared_with_user_id} className="rounded-md border border-white/10 p-3">
                  <div className="font-medium">{s.username}</div>
                  {s.email && <div className="text-xs text-slate-400">{s.email}</div>}
                  {confirmRevoke === s.shared_with_user_id ? (
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <span className="text-xs text-slate-400">Remove {s.username}?</span>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <select
                        className="flex-1 rounded-md border border-white/10 bg-slate-900/60 px-2 py-1 text-sm text-white"
                        value={s.permission}
                        disabled={busyUser === s.shared_with_user_id}
